...
const formSchema = z.object({
  nip: z.string().max(10, 'NIP maksimal 10 karakter').nonempty('NIP harus diisi'),
  name: z.string().max(50, 'Nama maksimal 50 karakter').nonempty('Nama harus diisi'),
  accountNumber: z.string().max(20, 'Nomor rekening maksimal 20 karakter').nonempty('Nomor rekening harus diisi'),
  bank: z.string().max(20, 'Nama bank maksimal 20 karakter').nonempty('Nama bank harus diisi'),
  phoneNumber: z.string().max(15, 'Nomor telepon maksimal 15 karakter').nonempty('Nomor telepon harus diisi'),
  address: z.string().nonempty('Alamat harus diisi').nonempty('Alamat harus diisi'),
  creditLimit: z.coerce.number().nonnegative('Limit piutang tidak boleh negatif').min(1, 'Limit piutang harus diisi'),
...
const TambahAnggota = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    dataUnique.map((data: dataUniqueType) => {
      if (data.nip === form.nip) {
        ...
        setErrors((prev) => ({ ...prev, accountNumber: 'Nomor rekening sudah terdaftar' }));
      }
    });
    ...
    try {
      ...
      const url = paramId ? `http://localhost:3000/api/member/${paramId}` : 'http://localhost:3000/api/member';
  ...
  return (
    <div className='max-w-4xl mx-auto p-6 rounded-2xl shadow border-gray-300 mt-24 bg-white'>
      <h2 className='text-xl font-semibold mb-6'>{paramId ? 'Edit Anggota' : 'Form Anggota Baru'}</h2>
      ...
      >
        ...
        <div>
          ...
          />
          {errors.nip && <p className='text-sm text-red-500 mt-1'>{errors.nip}</p>}
        </div>
